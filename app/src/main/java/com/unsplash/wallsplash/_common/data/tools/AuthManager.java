...
public class AuthManager
    ...
    private AuthManager() {
        ...
        SharedPreferences sharedPreferences = WallSplashApplication.getInstance()
    ...
    private void updateVersion() {
        int versionNow = WallSplashApplication.getInstance()
                .getSharedPreferences(PREFERENCE_NAME, Context.MODE_PRIVATE)
                .getInt(KEY_VERSION, 0);
        int buildTypeNow = WallSplashApplication.getInstance()
                .getSharedPreferences(PREFERENCE_NAME, Context.MODE_PRIVATE)
                .getInt(KEY_BUILD_TYPE, BUILD_TYPE_RELEASE);
        String token = WallSplashApplication.getInstance()
                .getSharedPreferences(PREFERENCE_NAME, Context.MODE_PRIVATE)
        ...
        if ((versionNow < VERSION_CODE || buildTypeNow != CORRECT_BUILD_TYPE)
            ...
            Toast.makeText(
                    ...
                    WallSplashApplication.getInstance().getString(R.string.feedback_please_login),
                    Toast.LENGTH_LONG).show();

            SharedPreferences.Editor editor = WallSplashApplication.getInstance()
